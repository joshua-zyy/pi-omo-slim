...
The Agent will ask where to place the repository, obtain approval for the exact clone command, and then continue from the local installation guide. That guide separately instructs the Agent to inspect the Pi environment, ask how models should be configured, back up affected configuration, install missing dependencies, copy the project files, merge the strict subagent settings, and verify the result.
